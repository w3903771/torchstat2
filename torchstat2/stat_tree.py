...
class StatNode(object):
    def __init__(self, name=str(), parent=None):
        ...
        self.parent = parent
        self.children = list()
        self.module_type = None  # Store module type for better naming
    ...
    @property
    def Memory(self):
        # Create a new list to avoid modifying the original tuple/list
        if isinstance(self._Memory, tuple):
            total_Memory = [self._Memory[0], self._Memory[1]]
        else:
            total_Memory = [self._Memory[0], self._Memory[1]]
        
        for child in self.children:
            child_memory = child.Memory
            total_Memory[0] += child_memory[0]
            total_Memory[1] += child_memory[1]
