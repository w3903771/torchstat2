...
def report_format(collected_nodes):
    ...
    for node in collected_nodes:
        # Improve module name display by combining name and type
        name = node.name
        # If the name is just a number (from nn.Sequential), try to make it more meaningful
        if hasattr(node, 'module_type') and node.module_type:
            if name.isdigit():
                name = f"{node.module_type}"
            else:
                name = f"{name}({node.module_type})"
    ...
    total_df = pd.Series([' ', ' ', ' ', total_parameters_quantity, total_memory,
                          total_operation_quantity, total_duration, total_flops,
                          total_mread, total_mwrite, total_memrw],
                         index=['module name', 'input shape', 'output shape', 'params', 'memory(MB)', 'MAdd', 'duration[%]', 'Flops',
                                'MemRead(B)', 'MemWrite(B)', 'MemR+W(B)'],
                         name='total')
    df = pd.concat([df, total_df.to_frame().T], ignore_index=True)
